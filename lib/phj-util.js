...
var qrcodeCanvas = function qrcodeCanvas(obj) {
    ...
    _qrcode2.default.toCanvas(document.getElementById(cid), text, options, function (error) {
        if (error) {
            console.error(error);
            return;
        }
...
var qrcodeImg = function qrcodeImg(obj) {
    ...
    // render
    options.width = options.width || $div.offsetWidth;
    _qrcode2.default.toDataURL(text, options, function (error, url) {
        if (error) {
            console.error(error);
            return;
        }
...
var qrcodeSvg = function qrcodeSvg(obj) {
    ...
    // render
    options.width = options.width || $div.offsetWidth;
    _qrcode2.default.toString(text, options, function (error, svg) {
        if (error) {
            console.error(error);
            return;
        }
